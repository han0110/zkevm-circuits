...
mod test {
    use crate::evm_circuit::{
        ...
        test::{
            rand_bytes, rand_fp, rand_range,
            run_test_circuit_incomplete_fixed_table,
        },
        witness::{
            Block, Bytecode, Call, CodeSource, ExecStep, Rw, RwMap, Transaction,
        },
    };
    use bus_mapping::{
        address,
        eth_types::{self, Address, ToWord, Word},
        evm::OpcodeId,
    };
                ...
                steps: vec![
                    ExecStep {
                        rw_indices: vec![
                            ...
                            (RwTableTag::CallContext, 10),
                            (RwTableTag::CallContext, 11),
                            (RwTableTag::CallContext, 12),
                            (RwTableTag::Account, 4),
                            (RwTableTag::Account, 5),
                        ],
    ...
    fn begin_tx_gadget_rand() {
        ...
        test_ok(
            mock_tx(
                Some(
                    Word::from_little_endian(&rand_bytes(32))
                        % one_hundred_ether,
                ),
                None,
                None,
