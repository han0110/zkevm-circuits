...
type Block struct {
	...
	HistoryHashes map[uint64]*hexutil.Big `json:"history_hashes"`
}
...
func TraceTx(block Block, accounts map[common.Address]Account, tx Transaction) (*ExecutionResult, error) {
	...
	blockCtx := vm.BlockContext{
		...
		GetHash: func(n uint64) common.Hash {
			if hash, ok := block.HistoryHashes[n]; ok {
				return common.BigToHash(toBigInt(hash))
			}
			return common.Hash{}
		},
		Coinbase:    block.Coinbase,
		BlockNumber: toBigInt(block.BlockNumber),
